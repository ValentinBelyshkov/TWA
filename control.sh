...
#
# Components: slam, relay, publisher, publisher:folder, publisher:realsense, all
...
show_logs() {
    ...
    if [ "$comp" = "all" ]; then
        ...
        for key in slam relay publisher; do
...
main() {
    ...
    if [ -z "${COMPONENTS[$component]}" ] && [ "$component" != "publisher" ]; then
        ...
        echo "Valid: slam, relay, publisher, publisher:folder, publisher:realsense, all"
        exit 1
    fi
