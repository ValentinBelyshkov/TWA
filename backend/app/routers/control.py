...
async def control_terraslam_component(action: ComponentAction, request: Request) -> CommandResponse:
    ...
    if action.component == "all" and action.action in ("start", "restart", "stop", "kill"):
        ...
        # Stop conflicting publisher mode first (gateway kill)
        for comp in others:
            try:
        ...
        # Perform requested action on targets
        for comp in target_components:
            try:
                # For publishers, set path before start
    ...
    # Publisher start requires folder path
